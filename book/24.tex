...
\begin{proof}
	...
	同时，由 $f(x) - \lambda_i$ 的分解，可知 $(A - q_1 I_n) \cdots (A - q_s I_n) \vec v_i = \vec 0$。注意到，若 $(A - q_2 I_n) \cdots (A - q_s I_n) \vec v_i \ne \vec 0$，则 $(A - q_2 I_n) \cdots (A - q_s I_n) \vec v_i$ 是 $A$ 的一个特征向量，对应特征值为 $q_1$，此时结论成立。

	否则，若 $(A - q_3 I_n) \cdots (A - q_s I_n) \vec v_i \ne \vec 0$，则 $(A - q_3 I_n) \cdots (A - q_s I_n) \vec v_i$ 是 $A$ 的一个特征向量，对应特征值为 $q_2$，此时结论成立。

	否则，继续照此进行检查，直到结论成立。最后一个式子可能是 $(A - q_s I_n) \vec v_i = \vec 0$，由于 $\vec v_i \ne \vec 0$，故在该情况下一定可以得出结论：$\vec v_i$ 是 $A$ 的一个特征向量，$q_s$ 是对应的特征值。
\end{proof}
...
\begin{definition}{代数重数,重数}
	把 $\lambda_i$ 作为 $A$ 的特征多项式的根的重数 $l_i$ 叫做 $\lambda_i$ 的\emph{代数重数}，简称为\emph{重数}。
\end{definition}
